...
public class DialogPanel extends GridPanel {
	 ...
	 * @param location 
	 * @param placement one of NORTH_WEST, NORTH_EAST, SOUTH_WEST, SOUTH_EAST or CENTER
	 */
	public void showInDialog(Window owner, Point location, int placement) {
		JDialog dialog = createDialog(owner);
		...
		case SwingUtilities.SOUTH_EAST:
			dialog.setBounds(new Rectangle(location.x - size.width, location.y + size.height, size.width, size.height));
			break;
		case SwingConstants.CENTER:
			dialog.setBounds(new Rectangle(location.x - size.width / 2, location.y + size.height / 2, size.width, size.height));
			break;
		}
		dialog.setVisible(true);
	}
